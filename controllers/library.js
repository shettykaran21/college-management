exports.getAllBooks = async (req, res) => {
  ...
  const [rows, fields] = await connection.query(
    'SELECT * FROM library ORDER BY ' + tag + ' ' + order
  );
...
exports.addBook = async (req, res) => {
  ...
  console.log(post);
  const [rows, fields] = await connection.query('INSERT INTO library SET ?', [
    post,
  ]);
  res.status(200).json(rows);
};
...
exports.deleteBook = async (req, res) => {
  ...
  const [rows, fields] = await connection.query(
    'delete from library where bname = ? and edition = ' + edition,
    [bname]
  );
  res.status(200).json(rows);
};
...
exports.updateBook = async (req, res) => {
  ...
  const [rows, fields] = await connection.query(
    'update library set bname = ?, author = ?, edition = ' +
      edition +
      ' where bookid = ' +
      bookid,
    [bname, author]
...
exports.searchBooks = async (req, res) => {
  ...
  filter = '%' + filter + '%';
  const [rows, fields] = await connection.query(
    ' SELECT * FROM library WHERE ' + tag + ' LIKE ?',
    [filter]
  );
  res.status(200).json(rows);
};
