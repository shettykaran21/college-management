exports.getAllProjects = async (req, res) => {
  ...
  const [rows, fields] = await connection.query(
    'select rp.pname, rp.p_desc, f.fname from (research_proj rp inner join research_faculty rf on rp.pid = rf.pid) inner join faculty f on rf.fid = f.fid ORDER BY ' +
      tag +
      ' ' +
      order
  );
...
exports.searchProject = async (req, res) => {
  ...
  filter = '%' + filter + '%';
  const [rows, fields] = await connection.query(
    'select rp.pname as pname, rp.p_desc as p_desc, f.fname as fname from (research_proj rp inner join research_faculty rf on rp.pid = rf.pid) inner join faculty f on rf.fid = f.fid WHERE ' +
      tag +
      ' LIKE ?',
    [filter]
  );
