...
function SingleDepartment() {
  ...
  async function getData() {
    try {
      ...
      const { dname, fname, email, noOfCourses, noOfStudents, noOfFaculties } =
        await doc.json();
      setDname(dname);
      setHod({ fname, email });
    ...
    try {
      ...
      if (action) {
        const doc = await fetch(
          `/api/department/get/${dname}?tag=${tag}&order=${order}`
        );
        const department = await doc.json();
        setDepartment(department);
      } else {
        const doc = await fetch(
          `/api/department/get/${dname}/course?tag=${tag}&order=${order}`
        );
